...
def ensure_machine_identity():
    ...
    cứng. Nếu chưa có, tính qua get_hardware_id() (machine_guid -> serial,
    bios_uuid -> uid) rồi lưu lại. Motherboard Serial vẫn được hardware_id.py
    đọc/trả về nhưng không dùng làm machine_serial.

    Trả về (serial, uid) hiện tại — có thể là (None, None) nếu Windows/BIOS
    không trả ID; nơi gọi (dialog đăng ký) tự quyết định chặn nút
    ...
    new_serial = serial or info.get("machine_guid")
